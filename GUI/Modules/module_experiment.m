...
%             % happpywhy
%             if type == 5
%                 if a==size(obj.data.Result,2)
%                     hywfig1 = figure; figSize =  [2 7 8 8];hold on;
%                     set(hywfig1,'Units','centimeters', 'Position',figSize);
%                     MedgecT={'b','r','g','m','y','c','k'};
%                     linesT={'-','--','-','-.'};
%                     fsize=10;
%                     for ia=1:a
%                         switch metric
...
%                             return;
%                         otherwise
%                             data(:,1) = [obj.data.Result{p,ia,existing(1)}.result{:,1}]';
%                             data(:,2) = mean(obj.data.metricValue(p,ia,@obj.Metric,metric,1),1)';
%                         end
%                         plot(data(:,1),data(:,2),'Linestyle',linesT{ia},'Linewidth',1.2,'Color',MedgecT{ia});
%                     end
%                     xlabel('N_{samples}'); ylabel('NHV');
%                     set(gca,'XTickMode','auto','YTickMode','auto','View',[0 90],...
%                         'FontSize',fsize,'FontName','Times new roman');
%                     axis tight;
%                     lgdia = legend('MOEGO-Penalty','MOEGO-LSSVM','MOEGO-wLSSVM','MOEGO-Kriging', ...
%                         'Location','Northoutside','NumColumns',2);
%                     lgdia.Box='off';
%                     figname = sprintf('./Data/Figures/History %s on %s',obj.data.Algorithms{a},obj.data.Problems{p});
%                     savefig(figname);
%                     export_fig([figname '.tif'],'-r1200');
%                     export_fig([figname '.eps']);
%                 else
%                     switch metric
%                         case {'Run times','runtime'}
%                             return;
