function [Score,PopObj] = HyperV1(PopObj,PF)
% <metric> <max>
...
%     PopObj = (PopObj-repmat(fmin,N,1))./repmat((fmax-fmin)*1.1,N,1)
    PopObj(any(PopObj>1,2),:) = []; % delete any popobjdayu1dezhi
...
function S_ = Add(cell_,S) % ruguo di er lie value equal, ze jiang diyilie yuansu xiangjia
    n = size(S,1);
    m = 0;
    for k = 1 : n
